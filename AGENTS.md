...
## SimulationGears ownership

SimulationGears_for_SpaceNav is primarily a MATLAB library for spacecraft
navigation simulation. Active MATLAB code lives under `matlab/`, tests under
`tests/matlab/`, and MEX builders under `matlab/builders/mex/`. Generated MEX
artifacts belong under `matlab/mex/` and must never pollute the repository root.

The retained root CMake, C++, CUDA, Python, wrapper, container, and editor
infrastructure originates from `cpp_cuda_template_project`. Import reviewed
shared implementations from that donor, then preserve this repository's
identity, dependencies, options, source/test composition, data policy, and
intentional removals through explicit tailoring. Never independently rewrite a
shared donor helper or restore an absent donor facility solely for parity.

Nested libraries and sibling repositories have separate ownership. Do not
modify dependency checkouts, gitlinks, or sibling worktrees unless the task
explicitly includes their owning repository.

### Primary commands

- `run('matlab/SetupSimGears.m')` adds supported MATLAB sources to the path.
- `matlab -batch "cd('/home/peterc/devDir/SimulationGears_for_SpaceNav'); run('matlab/SetupSimGears.m'); addpath(genpath('tests/matlab')); results = runtests('tests/matlab', 'IncludeSubfolders', true); assert(all([results.Passed]));"` runs the MATLAB suite.
- `./build_lib.sh` configures, builds, and tests the C++-first library.
- `./build_lib.sh -p -m` enables the configured Python and MATLAB wrappers.
- Run focused subsystem tests before the broader suite and report MATLAB
  assertions separately from wrapper teardown warnings or crashes.

<!-- ros2-overlay-begin -->
## Optional ROS 2 Overlay

See `doc/ros2_overlay.md` before changing the optional four-package ROS 2 Jazzy
overlay. `./build_lib.sh` is the C++-first library entry point and never needs
ROS. `./build_ros2.sh --clean` is the overlay build and test entry point.

Keep ROS-related changes confined to `ros2/` plus the documented root helpers,
docs, tests, markers, and the single ROS overlay workflow. Preserve package
prefix `simulation_gears`, lifecycle autostart in both launch paths, private
service/topic names, root and namespaced coverage, and the exact packages
`simulation_gears`, `simulation_gears_interfaces`, `simulation_gears_ros`, and
`simulation_gears_spinup`.
<!-- ros2-overlay-end -->
...
## SimulationGears-specific safeguards

- Preserve code-generation-safe fixed MATLAB struct shapes and field order
  where downstream MEX builds consume them.
- Test model-selection contracts using payloads containing all relevant data so
  data availability cannot be confused with selection policy.
- Preserve required Eigen3 3.4 and Threads, optional CUDA/OptiX/OpenGL/TBB,
  quiet OpenCV discovery, standalone submodule defaults, and the current
  four-workflow topology unless the user requests a contract change.
- Do not import donor conformance verifiers into ordinary project CTest. Use
  fresh disposable builds and consumers for configuration, installation,
  package, feature-matrix, and nested-consumption acceptance.
- Use `rg` or `rg --files` for searches and `apply_patch` for manual edits.
  Keep plans under `doc/developments/` synchronized with verified status.

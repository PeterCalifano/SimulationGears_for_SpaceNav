...
Purpose:
  Build the optional SimulationGears ROS 2 overlay in ros2/ with colcon. The
  standalone C++ library still builds with ./build_lib.sh and never requires ROS.
...
Examples:
  ./build_ros2.sh --clean
  ./build_ros2.sh --packages-select simulation_gears simulation_gears_interfaces
...
run_colcon_build() {
  ...
  build_cmd_+=(
    ...
    "-DSIMULATION_GEARS_ENABLE_CUDA=${cuda_flag_}"
    "-DSIMULATION_GEARS_ENABLE_OPTIX=${optix_flag_}"
    "${cmake_args[@]}"
  )

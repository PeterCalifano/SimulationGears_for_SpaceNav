classdef testScenarioDataManifests < matlab.unittest.TestCase
    % Verify tracked manifest metadata and shared external asset resolution.
    %
    % 21-09-2026  Pietro Califano, Codex gpt-5.6  Cover shared truth-render assets.
    ...
    methods (Test)
        ...
        function testManifestValidationChecksLocalShapeAssetWhenRequested(testCase)
            ...
            testCase.verifyError(@() ValidateScenarioDataManifest(strManifest, "Eros", ...
                charAssetRootPath=string(fixture.Folder), bRequireLocalAssets=true), ...
                "ValidateScenarioDataManifest:MissingAsset");
        end

        function testResolveDetailedTargetAssetBundles(testCase)
            charAssetRoot = testScenarioDataManifests.assetRoot_();

            objApophisAssets = ResolveTargetAssetBundle("Apophis", ...
                charAssetRootPath=charAssetRoot);
            objBennuAssets = ResolveTargetAssetBundle("Bennu", ...
                charAssetRootPath=charAssetRoot);

            testCase.verifyEqual(objApophisAssets.charShapeFormat, "obj");
            testCase.verifyEqual(objApophisAssets.enumShapeInputUnits, EnumLengthUnits.km);
            testCase.verifyTrue(isfile(objApophisAssets.charShapePath));
            testCase.verifyEqual(objBennuAssets.charShapeFormat, "obj");
            testCase.verifyTrue(objBennuAssets.bLoadMaterials);
            testCase.verifyEqual(objBennuAssets.enumAlbedoMapping, ...
                EnumSurfaceMapMapping.MESH_UV);
            testCase.verifyTrue(all(isfile(objBennuAssets.charAlbedoMapPaths)));
        end
        ...
        function testAllManifestBackedScenariosHaveAlignedDefaults(testCase)
            ...
            for charScenarioName = CScenarioRegistry.ListSupportedScenarios()
                ...
                testCase.verifyEqual(string(strAssets(idxDefaultShape).local_path), ...
                ...
                if any(strcmp(string(strManifest.tags), "shape_runnable"))
                    testCase.verifyTrue(logical(strAssets(idxDefaultShape).required_for_shape_runnable), ...
        ...
        function testRegisteredSpiceMetaKernelIsDeclaredByManifest(testCase)
            ...
            strAssets = NormalizeManifestStructArray(strManifest.assets);
            cellAssetPaths = string({strAssets.local_path});
    ...
    methods (Static, Access = private)
        function strManifest = buildValidManifest_()
            strAsset = struct( ...
                ...
                "asset_type", "shape", ...
                "local_path", "assets/bodies/eros/shape/near-a-msi-5-erosshape-v1_0_64q.bds", ...
        ...
        function charAssetRoot = assetRoot_()
            charAssetRoot = ResolveSimulationRenderingAssetsRoot();
        end
    end
end

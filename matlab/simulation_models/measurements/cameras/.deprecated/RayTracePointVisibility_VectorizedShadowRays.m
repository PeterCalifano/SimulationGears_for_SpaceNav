function [bAllPointsVisibilityMask, dProjectedPoints_UV] = RayTracePointVisibility_VectorizedShadowRays(ui32PointsIdx, ...
                                                                                                        dPointsPositions_TB, ...
                                                                                                        strTargetBodyData, ...
                                                                                                        strCameraData, ...
                                                                                                        dSunPosition_TB, ...
...
dTriangVerticesArray1 = dVerticesPos(:, ui32triangVertexPtr(1, :));
dTriangVerticesArray2 = dVerticesPos(:, ui32triangVertexPtr(2, :));
dTriangVerticesArray3 = dVerticesPos(:, ui32triangVertexPtr(3, :));

ui32NumOfTriang = uint32(size(ui32triangVertexPtr, 2));
...
parfor idL = 1:i32NumOfPointsToTrace
    ...
    [bIsIntersected, ~, ~, ...
        dIntersectDistance, ~] = RayTriangleVectorizedIntersect_MollerTrumbore(dCamPosition_TB, ...
                                                                                         ...
                                                                                         ui32NumOfTriang, ...
                                                                                         bTwoSidedTest); % Normal ray, two-sided test

    if (dRayToPointsFromCamNorm(idL) - dIntersectDistance) > eps('single') && bIsIntersected == true
    ...
    elseif (dRayToPointsFromCamNorm(idL) - dIntersectDistance) <= eps('single') && bIsIntersected == true
        % Intersection farther than or very near point detected --> point visible by camera
...
for id = 1:i32NumTrianglesInSubset
    ...
    if all(i32triangVertPtr1_tmp ~= ui32TestPointIdx & i32triangVertPtr2_tmp ~= ui32TestPointIdx & ...
        ...
        [bLightOcclusion_tmp, ~, ~, ...
            ...
            true); % Shadow ray, two-sided test

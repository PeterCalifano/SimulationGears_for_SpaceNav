function [dxStateHistory, dTimeGrid, strStatistics] = PropagateAdaptiveStep( ...
    fcnStateDerivative, dTimeSpan, dxInitialState, dInitialStep, ...
    strDynParams, strModelConfigFlags, settings) %#codegen
%% SIGNATURE
...
dStageValues(:, 2) = EvaluateDerivative_(fcnStateDerivative, ...
    ...
    dxCurrentState, strDynParams, strModelConfigFlags);
...
function dxDerivative = EvaluateDerivative_( ...
    fcnStateDerivative, dEvaluationTime, dxEvaluationState, ...
    dxReferenceState, strDynParams, strModelConfigFlags)
% Validate a raw RHS result before stage-buffer assignment can expand it.
dxDerivative = fcnStateDerivative( ...
    dEvaluationTime, dxEvaluationState, strDynParams, strModelConfigFlags);

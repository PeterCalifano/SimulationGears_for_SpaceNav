...
switch testCase
    case 1
        ...
        % var: state vector
        N = 50;
        var = MX.sym('var', N*11+2, 1);
        state_i_rot = MX.sym('IC', 1, 7);
        par = MX.sym('params', 1, 9);
        % [C, Ceq, ~, ~] = land_nonlincon_soft(var, state_i_rot, par, N);
...
%         dUVdxState_fcn.export_code('matlab','dUVdxState_fcn.m') % Does not work with inputs equal to zero
    ...
    case 5
        ...
        %% TODO: fast validation function using cubature points?
        finalRef = odeOut(end, :)';
...
% function [rNext, vNext] = eulerIntegratorDA(mu, rk, vk, dt, np)
% 
% rNext = rk + nlinfun(rk, vk, mu, np)*dt;
% 
% vNext = vk + nlinfun(rk, vk, mu, np)*dt;
% 
%     function fx = nlinfun(r, v, mu, np)
%         r = np.linalg.norm(r);
%         fx = [v(1);
%             v(2);
%             v(3);
